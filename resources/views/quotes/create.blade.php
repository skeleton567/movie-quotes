        ...
        <x-list-wrap name="{{__('text.new_quote')}}">
            <form class="space-y-6" action="{{route('quotes.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <x-form.textarea name="name.en">{{old('name_en')}}</x-form.textarea>
                    <x-form.error name="name_en"/>
                </div>

                <div>
                    <x-form.label for="name_en" name="{{__('text.quote_georgian')}}"/>
                    <x-form.textarea name="name.ka">{{old('name_ka')}}</x-form.textarea>
